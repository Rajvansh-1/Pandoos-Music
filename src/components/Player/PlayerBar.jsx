...
export default function PlayerBar() {
  ...
  return (
    <footer className={`player-bar luxury-player-bar${isPlaying ? ' is-playing' : ''}`}>
      ...
      {isPlaying && (
        <div className="pb-visualizer">
          {[...Array(28)].map((_, i) => (
            <div key={i} className="pb-vis-bar" style={{ animationDelay: `${(i * 0.07) % 0.5}s` }} />
          ))}
        </div>
      )}

      {/* ── Left: Now Playing ── */}
      <div className="pb-info" onClick={actions.toggleFullscreen}>
        {currentSong ? (
          <>
            {currentSong.coverUrl ? (
              <img className="pb-art luxury-art" src={currentSong.coverUrl} alt={currentSong.title} />
            ) : (
              <div className="pb-art luxury-art" style={{ background: 'var(--bg-active)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem' }}>
                🎵
              </div>
            )}
            <div style={{ minWidth: 0 }}>
              <div className="pb-title" title={currentSong.title}>{currentSong.title}</div>
              <div className="pb-subtitle" title={currentSong.artist}>{currentSong.artist}</div>
            </div>
            <div onClick={e => e.stopPropagation()}>
              <LikeButton liked={isLiked} onClick={() => actions.toggleLike(currentSong.id)} size={20} />
            </div>
          </>
        ) : (
          <span style={{ color: 'var(--text-muted)', fontSize: '0.85rem', fontWeight: 600, paddingLeft: 8 }}>
      ...
      <div className="pb-controls">
        <div className="pb-buttons">
          {/* Shuffle */}
          <button
            className={`icon-btn pb-icon-btn${shuffle ? ' active' : ''}`}
          ...
          {/* Prev */}
          <button className="icon-btn pb-icon-btn" onClick={actions.prevTrack} title="Previous">
            <PrevIcon />
          </button>

          {/* Play/Pause */}
          <button
            className="pb-btn-play luxury-play-sm"
          ...
          {/* Next */}
          <button className="icon-btn pb-icon-btn" onClick={actions.nextTrack} title="Next">
            <NextIcon />
          </button>

          {/* Repeat */}
          <button
            className={`icon-btn pb-icon-btn${repeat !== 'none' ? ' active' : ''}`}
      ...
      <div className="player-extras">
        <button
          className={`icon-btn pb-icon-btn${state.showQueue ? ' active' : ''}`}
        ...
        <button
          className="icon-btn pb-icon-btn"
        ...
        <div className="volume-bar">
          <button className="icon-btn pb-icon-btn" onClick={actions.toggleMute} title={isMuted ? 'Unmute' : 'Mute'}>
          ...
          <input
            ...
            aria-label="Volume"
            style={{
              background: `linear-gradient(to right, var(--ambient-primary, #4ade80) ${(isMuted ? 0 : volume) * 100}%, rgba(255,255,255,0.1) ${(isMuted ? 0 : volume) * 100}%)`
            }}
          />
        </div>
      </div>
...
function PrevIcon()    { return <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z"/></svg>; }
function NextIcon()    { return <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z"/></svg>; }
